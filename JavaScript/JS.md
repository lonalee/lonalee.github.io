...
3. TodosComponent의 template 생성
  ...
  addTodo에서 입력 value 지우기 (양방향 바인딩으로 처리하자, ***컴퍼넌트 클래스 안에서 input 요소를 query하는 것***은 Angular way가 아니다. input의 value를 지우기 위해서 ''를 할당하려면 input요소에 접근해야 하는데 이 방법은 document.queryselector 하거나 양방향 바인딩 외에는 없다.)
  상태 데이터 변화에 따라 템플릿을 재생성하는 것은 불필요한 것.
  양방향 : ***[(ngModel)]***="content"
***양방향 바인딩 content라는 프로퍼티를 만들어서 바인딩 함. 이것으로 addTodo() 메소드에서의 참조?가 약간 달라진다 (더 간결해짐)***
양방향 바인딩은 form 형식의 요소에서 주로 사용한다. 템플릿 & 컴퍼넌트가 상태 변화를 상호 반영하기 위해서 사용하는 것이다.
...
>todos-input에서 이벤트 발생하면 todos-container에 알람, 바뀐 배열을 todos-list에 전달해서 렌더링
컴퍼넌트 간의 interaction (컴퍼넌트 간의 부모자식 관계)

템플릿 참조 변수 / <input ~~addTodo(input.value) #input> 돔 요소 안에 < #변수명 > 선언 방식 --> performance 이슈가 있을 수도 있음

----
## 분리 작업
input & list의 컴퍼넌트를 분리한다.
ng g c 컴퍼넌트명
최초에는 생성된 컴퍼넌트를 사용하는 부모 요소가 없다. 그래서 todos-container의 템플릿에 커스텀 태그를 넣자.

컨테이너에 addTodo를 정의해야 add-list에서도 사용할 수 있음

부모 =>> 자식 컴퍼넌트 데이터 전달은 ***프로퍼티 바인딩을 사용***
자식 컴퍼넌트가 데이터를 받으려면 ***별도 문법 사용***
addTodo 실행을 부모에게 요청할 때는 ***이벤트 바인딩 사용***
컨테이너에 있는 todos의 참조를 todos-input & list에 전달해준다.
    부모 =>> 자식) [todos]="todos" : ***참조를 전달***  call by reference
    @Input(여기에는 부모가 주는 이름을 쓸 수도 있다--) todos: Todo[] (데코레이터-함수)  부모가 전달한 것을 todos-input property-라는 곳에 담겠다. import도 해야 함.
    인터페이스도 별도 설정해야 한다. (컨테이너에 종속시키지 않기 위해서)
    ng g i todo.interface
    todo.interface 모듈을 import하자
이벤트 발생은 이벤트 바인딩으로 자식 =>> 부모 요청이 가야 함.
    다른 컴퍼넌트에 전파할 이벤트를 또 생성
    @Output() toggleTodoCompleted = new eventEmitter<넘버자료형을 주자 number>() (임포트도 해야 함)
    toggleTodoCompleted.emit(todo.id)
    eventEmitter 이벤트를 부모에게 방출한다.
    부모 컴퍼넌트에서 이벤트 발생되는 요소에서 (toggleTodoCompleted)="toggleTodoCompleted($event)
    자식 => 부모 프로퍼티 바인딩 못하는 이유? 부모의 태그를 템플릿에 갖고 있지 않기 때문에.
컨스트럭터 실행 시점에는 부모가 아직 전달하지 않았음, ngOnInit 실행 시점에 전달 됨.
todos-list에서는 Oninit, constuctor 필요 없음
todos-input에서는
    content(이벤트바인딩 프로퍼티) = '';
...
-------
***phase0***
todos-container, 하위에 todos-list, todos-input 컴퍼넌트를 생성 총 3개.
interface 파일은 todos-container 폴더에 만든다.
todos-container에는 addInput, toggleTodoCompleted, removeTodo 메소드를 정의 (메소드는 모두 컨테이너에 포함)
***phase1***
todos-input은 이벤트 발생 시 해당 데이터를 부모 컴퍼넌트에게 전달해야 함. (이벤트 바인딩 필요)
  todos-container에서는 템플릿에서 이벤트가 발생할 요소에 이벤트 바인딩을 정의한다.
  이벤트가 발생하는 컴퍼넌트에서는 템플릿에서 해당 요소에 메소드를 할당한다. (이벤트 타겟에 전달된 value를 전달하는 메소드)
  이 메소드는 이벤트 타겟의 value(사용자 입력값)를 인자로 전달 받아서 emit 프로퍼티에 할당한다.
  할당하기 전에는 당연히 eventEmitter로 객체를 생성해야 한다.
  여기서 Output하는 객체의 이름을 부모 컴퍼넌트에서 이벤트 바인딩으로 참조하고 있다.
    Enter Todo로 리스트 변경 완료.
***phase2***
toggleTodo : todos-list에 input으로 checkbox 생성, 여기에 이벤트 발생하면 컨테이너에 알려준다.
removeTodo

-------
shadow DOM 스타일 셀렉터
html -> 루트컴퍼넌트로 복사해서 주석 및 메타데이터 부분 제거
body 제외하고 나머지 css부분은 루트컴퍼넌트의 css로 복사.
angular.json 에서 글로벌 css 등록 부분 (style.css)  node_modules/boot~~~min파일을 쓴다.
파일의 경로를 angular.json의 style 부분에 명시한다.
...
----
## 커스텀 파이프 생성
탭 클릭에 따라 todos를 다시 렌더링한다는 개념이 아니고, zone.js가 변화를 감지할 때마다 view는 재 생성된다.
따라서 상태 데이터를 표시하는 로직이 있는 곳에 pipe를 명시한다.
todos.filter: {{ currentNavItem }} -All, Active, Completed 중 무엇인지?
todos 배열을 제어하는 것은 todos-filter.pipe 컴퍼넌트에서 작업한다. 여기에 transform에 경우에 따른 상태데이터 표시 로직을 정의한다.
탭은 All, Active, Completed는 li를 ngFor를 이용해서 생성한다.
  navItems, currentNavItem 사용해서 클래스를 동적으로 할당하는 방법
  "currentNav===navItem"
  currentNav All로 초기화하고, navItem이 ngFor로 인해서 순차적으로 넘어오면서 비교결과가 다른 요소(Active, Completed)는 클래스를 할당 받지 않는다.
  클릭되면 클릭된 navItem을 currentNav에 할당해서 비교결과가 같아지면 active 클래스가 할당된다.
todos | todosFilter : currentNav
todos를 대상으로 todosFilter 파이프를 사용하는데 옵션으로 currentNav 값을 넘겨준다. currentNav가 All, Active, Completed 중 무엇인지에 따라 if/switch로 로직 구현

----
## 최종 정리 (JS -> ANGULAR)

1. ng new : 프로젝트 생성 (-S, 스펙 파일 생성하지 말자)
- ng g c / ng g i : 컴퍼넌트 / 인터페이스 생성
2. /src/app 폴더 이동
- 루트 컴퍼넌트.html 작성 (body 요소 이하의 요소만 포함 시킨다 - 'app-root'의 내용 작성)
- 컴파일 시 webpack 구동, 테스트 : ng-serve
- 루트 컴퍼넌트.css 작성, body는 global css파일에 적용한다.
cf. :host { ~~ } : app-root를 css에서 선택할 때 (최상위 요소를 선택하는 것)
- bootstrap install : npm i bootstrap@3.3.7
- 설치 후 적용 : 외부 라이브러리(파일) 적용 방법? 'link 태그', '@import'
>예. index.html에 link 태그로 추가도 가능하나, angular.json에 추가한다. (설정파일 수정 시에는 적용하기 위해서 reboot해야 함)
styles.css에서 @import도 가능하다.
3. 페이지 로드 시 서버로부터 받은 데이터의 표시
 - 상태 데이터의 자료 구조 정의 Todo[] -> interface 화
 - Onit의 추상메소드 ngOnInit에서 데이터 초기화 발생. (서버와의 데이터 통신이 처리되는 곳)
 AppComponent class가 Angular에 의해 instance화 된다. -> constructor -> ngOnInit
 cf. @Input / Output
 Input 시점은 constructor 호출 후이다. 따라서 상태데이터 초기화는 constructor에 정의해도 되지만 Input/Output 데코가 있다면 부적절할 수 있음
- ngFor : built-in directive
- HTML요소들의 id는 DOM query를 위해 필요했으나 Angular에서는 필요 없음 (numbering하는 id는 예외)
- ngFor : 순회하면서 사용할 요소의 (지역)변수명 정의 (let todo of todos)
- label 요소 / input 요소의 id : property binding으로 todo.id를 할당
cf. ID할당 시 중복을 방지하려면? DB에서 보장되어야 함
- input 요소의 checked도 property binding
4. addTodo logic
- (keyup.enter)=메소드
cf. 메소드 정의 : ***역할, 용도의 명확화 -> 재료,요소,데이터의 정의***
- addTodo는 Todo 타입의 객체를 만들어야 한다.
- MaxId 메소드 : math.max(...this.todos.map(todo => todo.id))
  - 빈 배열일 때의 방어 코드 작성 (0/1의 truthy 여부로 Math.max 실행 여부를 결정)
- newTodo 객체를 담을 변수 생성, 타입은 Todo  const newTodo = {}
- todos에 newTodo를 추가 this.todos = [newTodo, ...this.todos]
- addTodo가 빈 문자열을 받을 때 방어 코드 : content가 falsy value이면 return
- Enter Todo에 '' 문자열 할당 : 템플릿의 변화가 view에도 적용이 된다.
  - ngModel로 content 프로퍼티를 양방향 바인딩하고 addTodo에 있던 인수($event.target.value)는 불필요하게 된다. 양방향 바인딩한 content를 addTodo에서 사용한다.  컴퍼넌트 클래스에서 content 프로퍼티 선언, 빈 문자열 초기화 -> this.content로 사용한다.
5. removeTodo logic
- todo의 id를 알아야 한다.
6. toggleTodo ligic
- change 이벤트로 잡아 보자.
- 객체의 일정 부분만 변경 : Object.assign / spread 연산자 {...todo} : 새로운 객체에 todo 프로퍼티를 풀어 넣는다. (복사)

7. TAB control
- HTML template에 중복 코드가 있다. -> ngFor를 사용할 수 있을 것이다. -> ngFor 하려면 배열이 있어야 함.
  - 템플릿에서 바뀌어야 하는 데이터는 All, Active, Completed
  - ngFor를 이용해서 해당 데이터를 담은 배열을 템플릿화 한다.
  - 현재 선택된 tab을 관리하는 프로퍼티를 추가한다. 이것은 앱 기동 시에는 All이다.
  - 단항 클래스 바인딩(가칭)으로 활성화되는 class name을 추가한다. 이 클래스 바인딩에는 조건식을 할당한다.
- 클릭 시 currentNav를 현재 선택된 navItem으로 바꿔준다. 이 때 클래스 바인딩이 적용되서 조건식 평가 후 클래스가 생성된다.
***cf. 마우스 클릭으로 currentNav가 바뀌면 emit해서 container에게 알려줘야 한다.***
***(click)="changeCurrentNav.emit(navItem)"***
- All일 때는 현재 todos, Active일 때는 non check, ...
    - 상태 데이터를 수정하는 방식이 아니다.
    - todos 배열(파이프 대상) | 정의한 파이프 이름: currentNavItem(옵션)
    - transform 정의 (todos: Todo[], NavItem: string)
      - 반드시 return 있어야 함. 여기서는 필터링된 배열을 리턴하자.
      - navItem이 string으로만 정의하는 것은 약함 (버그 차단 측면에서). All, Active, Completed 외에는 차단하기 위함.
          - 어떻게? 프로퍼티: '' | '' | '' = 'All'
          - type KEYWORD(typeScript 문법)
          type NavItem = '' | '' | ''; (union type) => pipe에서도 사용하므로 별도 파일화 (type.ts) export도 해야함.
          - ***결과적으로 NavItem 타입 이외의 데이터는 들어 올 수 없다.***
```
          transform ~~~
          return todos.filter(todo => {
            switch (navItem){
              case 'Active':  return !todo.completed;
                // (return은 반드시 필요. 화살표 함수에서의 중괄호 사용 시)
              case 'Completed':  return todo.completed;
              default:  return true // no-filterfing
            }
          })
```
>  ***PIPE***
todos에 새로운 배열의 참조를 할당한 이유? 원본 배열에서 직접 수정할 때(unshift 등)와 pipe의 작동 방식이 다르다.
파이프는 언제 동작하는가? 대상이 바뀌었다는 것을 감지할 때 동작. 참조가 바뀔 떄 대상 변화로 인지한다. 파이프를 쓰기 위해서는 todos에 새로운 참조를 할당해야 함.
변화 감지에도 부하를 줄이기 위해서는 새로운 참조를 할당하는 방식으로 코딩해야 함. 원본배열을 수정하는 방식은 비순수 프로그래밍 방식
배열을 다룰 때는 가급적 원본 배열을 수정하는 method는 지양하라.
순수 파이프 : 대상 값이 바뀌었을 때만 동작 (참조 변화는 x)
비순수 파이프 : pure: false (퍼포먼스 저하를 야기할 수 있음)

8. markAll logic
- this.todos = this.todos.map(todo => ({...todo, completed: true}))

9. clearCompleted logic
- todo => !todo.completed (true가 아닌 객체만 필터링하기 위함)

10. getCntActiveTodos(): number {
  return this.todos.filter(todo => !todo.completed).length;
}

# Directive
컴퍼넌트 내부에 있는, 다른 컴퍼넌트와 공유될 수 있는 기능을 directive라 한다.
어트리뷰트처럼 다른 컴퍼넌트에서도 사용할 수 있는 것.
```
...
cf. ngModel (directive) : input 요소(호스트 요소)의 참조를 알고 있기 때문에 자동으로 수행해줌.
host요소의 참조를 넘겨주는 것(constructor에 명시되는 매개변수에 의존성을 주입하는 것), 넘겨줘서 directive에 정의된 로직을 수행한다. angular에게 주입을 요청할 수 있는 것은 constructor 내부에 작성
ElementRef : angular에 의해 생성된 ElementRef class

cf. ngModel은 자식 컴퍼넌트에서 바인딩, 컴퍼넌트의 템플릿에서는 완전한 HTML을 갖도록 만들어야 한다.
content는 자식
addTodo는 부모에 정의
@Output addTodo
...
>비동기 데이터를 template에서 어떻게 사용할 것인가?
of : 배열을 observable로 리턴해 준다.
get 메소드는 observable을 리턴. (모든 http 메소드)

비동기 데이터 : get이 요청 즉시 처리 되지 않는 것. 이러한 경우에 undefined를 대상으로 다른 코드에서 작업을 처리하려고 한다. 어떻게 핸들링하는가?
대부분의 경우에는, 데이터 로딩 중이라는 등의 안내를 표시, 이렇게 하기 위해서 ***ngIf*** : 데이터가 오면 처리, 안 오면(else) 처리
ng-container : 부모 요소가 없을 때, 요소처럼 쓸 수 있는 디렉티브.
  ng-container> *ngIf="todos"; else loading </ng-container
  ng-template #loading> loading... </ng-template
app-root에서 컴퍼넌트 기동 이전에 할 수 있는 작업이 있음 (loading spinner 표시 등등)
컴퍼넌트를 만들지 않고 하드코딩해서 이전 작업을 정의할 수 있다.

# Component & Directive

input 프로퍼티 : input 데코레이터에 있음 (부모가 전달해준 값이 온다)
ngOnchanges
ngOnInit : 초기화 완료 시점 : 모든 프로퍼티의 초기화가 완료 되었다! (input 프로퍼티 포함)  OnInit이 갖고 있는 단 하나의 추상 메소드.
ng라는 prefix를 빼면 인터페이스, 인터페이스를 implement해서 각 인터페이스가 갖고 있는 추상 메소드를 구현한다.
초기 데이터를 갖고 올 때는 (class를 사용할 때는) constructor, 그러나 angular에서는 ngOninit

루트 컴퍼넌트 -> todos container 데이터 전달하여 input으로 받아본다.

# SERVICE
싱글턴 : 하나의 인스턴스로 메소드를 공유 (프레임워크가 해당 인스턴스를 관리한다는 전제) 별도의 메소드로 접근한다.
팩토리 : 인스턴스 생성을 대신한다.

서비스로 서버 데이터 수신하는 기능을 구현.
getTodos() {

}
서비스는 모듈에 별도 등록 필요 (providedIn 있으면 할 필요 없음)
**서비스 중재자 패턴**
...
class에도 프로토타입 개념이 적용된다.

http method
put 특정 데이터를 전부 수정, patch는 일부 수정
delete
...
HTML에서 보낼 때는 form (action / method), input에서 submit하게 되면 화면 전환이 발생하게 된다. 비동기방식이 아닌 것.

1. template driven form ; 비교적 단순한 form에 사용
2. reactive (class driven) form : 일반적으로 많이 사용


2. ReactiveFormsModule

formControlName - formControl 요소

## custom validator 생성

ng*** 클래스(touched, dirty, pristine 등)의 동적 생성 가능

MODAL : 팝업 떴을 때 백그라운드를 눌러도 변화가 없음.
(팝업이지만 전체 영역을 차지하게 만든다.)

# ngModule

ES6의 모듈과는 다른 것.
view의 부품화가 angular의 관심사.

>루트 모듈 (디스트럭처링으로 필요한 것만 {객체}로 담는다)
1. 브라우저에서 필요한 기능 : 구조 디렉티브, 바인딩에 필요한 것들
2. ngModule 데코레이터를 사용하기 위해 임포트함

  - 데코레이터함수 뒤에 오는 클래스를 장식한다. : 클래스 생성 시 컨스트럭터를 변형 시킨다.
  - 메타데이터 객체 : 클래스를 생성할 때 필요한 생성 정보를 담는다.
  - declararions : 모듈에 소속될 컴퍼넌트, 디렉티브, 파이프 등록 -> AppModule의 일원이 되었다. (애플리케이션 전역에서 사용할 수 있다- import 없이도)
  - imports : 모듈의 조합을 명시, AppModule과 조합될 다른 모듈을 선언.
  - providers : 서비스를 선언. 서비스 등록 방법(injectable 데코레이터 providedIn으로 작성해도 됨, )
  - bootstrap : 루트 컴퍼넌트를 명시. 모듈이 기동을 시작하면 루트 컴퍼넌트를 생성한다.

3. @angular : angular가 제공하는 라이브러리 모듈

# Routing

url path별로 필요한 component를 알려준다. 전체 HTML에서 필요한 부분(component)만 다시 렌더링한다. (URL 변경은 있지만 SPA)